...
#Main Menu images
title_img = pygame.image.load("img/main_menu/title.png").convert_alpha()
settings_img = pygame.image.load("img/main_menu/settings_button.png").convert_alpha()
play_img = pygame.image.load("img/main_menu/play_button.png").convert_alpha()
exit_img=pygame.image.load("img/main_menu/exit_button.png").convert_alpha()
level_select_img=pygame.image.load("img/main_menu/level_menu.png").convert_alpha()
menu_background_img=pygame.image.load("img/main_menu/menu_background.png")

#Level Menu images
level_1_img=pygame.image.load("img/main_menu/level_1.png").convert_alpha()
level_2_img=pygame.image.load("img/main_menu/level_2.png").convert_alpha()
level_3_img=pygame.image.load("img/main_menu/level_3.png").convert_alpha()
...
pizza_box=pygame.image.load("img/pizza_box.png")
pizza_box_img=pygame.transform.scale(pizza_box,(128,128))
pizza_box=menubutton.DrawMenu(800,50,pizza_box_img,1)
pizza_box_rect = pizza_box_img.get_rect()
pizza_box_rect.center = (800,50)
...
#Main Menu button
play_button=menubutton.DrawMenu(310,200,play_img,2.5)
settings_button=menubutton.DrawMenu(310,300,settings_img,2.5)
exit_button=menubutton.DrawMenu(510,300,exit_img,2.5)
level_select_buttton=menubutton.DrawMenu(510,200,level_select_img,2.5)
...
#Level Menu Button
level1_button=menubutton.DrawMenu(200,200,level_1_img,3)
level2_button=menubutton.DrawMenu(400,200,level_2_img,3)
level3_button=menubutton.DrawMenu(600,200,level_3_img,3)
...
player=MS.player
world=MS.world
level_complete=MS.level_complete
# Game loop
def game():
    global pause_menu,screen_scroll,bg_scroll,warning,main_menu,settings,level_complete,level,victory,world,lose,level_menu,start_game
    if not pause_menu:
        ...
        MS.world.draw()
        #show player health
        MS.health_bar.draw(MS.player.health)
        #show ammo
        draw_text('PEPPERONI: ', font, WOOD_BROWN, 10, 45)
        for x in range(MS.player.ammo):
        ...
        if MS.player.rect.colliderect(pizza_box_rect):
            level_complete = True
        ...
        if MS.player.alive:
            ...
            if MS.shoot:
                MS.player.shoot()
                MS.player.update_action(4)
            if MS.player.in_air:
                MS.player.update_action(2) # 2: jump
            elif moving_left or moving_right:
                MS.player.update_action(1) # 1: run/roll
            elif dash:
                MS.player.update_action(1) # can change to other animation 
            else:
                MS.player.update_action(0) # index 0: idle
            screen_scroll, MS.level_complete= MS.player.move(moving_left, moving_right, dash) # follows player speed # scrolling follows dashing TRUE or FALSE
...
while running: 
    ...
    if main_menu==True:
        ...
        if play_button.draw(screen):
            ...
            main_channel.pause()
        if level_select_buttton.draw(screen):
            pygame.time.wait(500)
            title.draw(screen)
            screen.blit(menu_background_img,(0,0))
            level_menu=True
        ...
        if level1_button.draw(screen):
            ...
            with open(f'level{level}_data.csv', newline='') as csvfile:
                ...
                for x, row in enumerate(reader):
                    for y, tile in enumerate(row):
                        world_data[x][y] = int(tile)
            MS.player.jump = False
            MS.player.shoot = False
            MS.player.in_air = False
            MS.player.dash = False
    elif stats==True:
        victory_channel.pause()
